...
const displayPost = async() => {
    ...
    const allPost = posts.map((post) => {
        ...
        return `
            <div class="post">
                <div style="display: ${post.username === username ? 'flex' : 'none'}"  id="action" class="actions">
                    
                    <button class="delete" data-id="${post._id}">Delete</button>
                   <a href="edit.html?id=${post._id}"> <button class="edit" data-id="${post._id}">Edit</button></a>
                </div>
                <h2 id="header">${post.postHeader}</h2>
                <div class="post-meta">by ${post.username} | ${formatted}</div>
